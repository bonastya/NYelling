...
public class MoveBackground : MonoBehaviour
{
	...
	public float thirdBGSpeed = 0.07f;

	public MeshRenderer fouthBG;
	public float fouthBGSpeed = 0.09f;

	public MeshRenderer fithBG;
	public float fithBGSpeed = 0.1f;

	private Vector2 savedFirst;
	private Vector2 savedSecond;
	private Vector2 savedThird;
	private Vector2 savedFouth;
	private Vector2 savedFith;
	...
	void Awake()
	{
		...
		if (fouthBG) savedFouth = fouthBG.sharedMaterial.GetTextureOffset("_MainTex");
		if (fithBG) savedFith = fithBG.sharedMaterial.GetTextureOffset("_MainTex");
	}
	...
	void Update()
	{
		...
		if (fouthBG) Move(fouthBG, savedFouth, fouthBGSpeed);
		if (fithBG) Move(fithBG, savedFith, fithBGSpeed);
	}
	...
	void OnDisable()
	{
		...
		if (fouthBG) fouthBG.sharedMaterial.SetTextureOffset("_MainTex", savedFouth);
		if (fithBG) fithBG.sharedMaterial.SetTextureOffset("_MainTex", savedFith);
	}
}